...
## Бизнес-модель проекта (кратко)

Electronics RFQ Agent решает задачу ускорения подготовки коммерческих предложений для дистрибьюторов электроники. Вместо ручной обработки RFQ инженерами по продажам (часы работы на один документ), проект автоматизирует:

- распознавание RFQ-документов (PDF/Excel/Word),
- сопоставление позиций с номенклатурой ERP,
- расчет черновика цены и статусов доступности,
- формирование структурированного результата для дальнейшей проверки и отправки клиенту.

Ценность для бизнеса:

- снижение затрат на рутинную обработку заявок,
- сокращение времени ответа клиенту (time-to-quote),
- повышение конверсии за счет более быстрых и единообразных ответов,
- возможность self-hosted внедрения для работы с чувствительными коммерческими данными.

Модель монетизации/внедрения:

- продукт как open-source библиотека и CLI для быстрой интеграции,
- подключение к существующим ERP через MCP-коннекторы (SAP, Epicor, Oracle, Dynamics),
- масштабирование в enterprise через кастомные коннекторы, поддержку и внутреннее развертывание.

## Базовый процесс работы в проекте

1. Пользователь передает RFQ-файл (или текст) в агент через API, CLI или MCP.
2. Парсер извлекает строки запроса: part number, количество и контекст по позиции.
3. Агент параллельно выполняет lookup каждой позиции в выбранной ERP-системе.
4. Для найденных позиций рассчитываются цены (включая логику маржи/порогов, если применимо).
5. Для ненайденных позиций фиксируются статусы и варианты замены (если доступны).
6. На выходе формируется черновик quote с итоговой сводкой (fill-rate, найдено/заменено/не найдено).
7. Результат может быть сохранен в JSON, проверен через audit и передан в следующий этап продаж.
